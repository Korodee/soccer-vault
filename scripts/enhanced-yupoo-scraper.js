...
class EnhancedYupooScraper {
  ...
  async init() {
    ...
    await this.page.route('**/*.{css,woff,woff2,ttf,eot,svg}', route => route.abort());
      ...
      const imageData = await this.page.evaluate(() => {
        ...
        selectors.forEach(selector => {
          ...
          elements.forEach(img => {
            ...
            if (src && 
                (src.includes('photo.yupoo.com') || src.includes('yupooimg.com')) && 
  ...
  async downloadImageWithBrowser(imageUrl, filePath) {
    ...
    try {
      ...
      try {
        ...
        console.log('    ⚠️ Could not screenshot image element, trying page screenshot');
      }
  ...
  async downloadImage(imageData, productSlug, imageIndex) {
    ...
    try {
      ...
      for (let attempt = 1; attempt <= this.retryAttempts; attempt++) {
        try {
          console.log(`    📥 Downloading image ${imageIndex + 1} (attempt ${attempt}/${this.retryAttempts})`);
      ...
      console.error(`    ❌ Failed to download image ${imageIndex + 1}:`, error.message);
      return null;
    }
  }
  ...
  async scrapeAlbums() {
    ...
    try {
      ...
      for (let i = 0; i < albumsToProcess.length; i++) {
        ...
        const imageData = await this.extractAlbumImages(album.href, album.title);
        ...
        if (imageData.length > 0) {
          ...
          for (let j = 0; j < imagesToDownload.length; j++) {
            const filename = await this.downloadImage(imagesToDownload[j], productSlug, j);
            if (filename) {
