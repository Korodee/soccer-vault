...
class ProductEnrichmentPipeline {
  ...
  async loadRawData() {
    try {
      ...
      console.log('💡 Make sure to run the scraper first: npm run scrape:enhanced');
      return { albums: [] };
    }
  }
  ...
  extractLeague(title) {
    ...
    if (club.includes('arsenal') || club.includes('chelsea') || club.includes('manchester') || 
        club.includes('leeds united') || club.includes('west ham') || club.includes('liverpool') ||
        club.includes('tottenham') || club.includes('newcastle') || club.includes('brighton') ||
        club.includes('crystal palace') || club.includes('aston villa') || club.includes('fulham') ||
        club.includes('brentford') || club.includes('wolves') || club.includes('everton') ||
        club.includes('nottingham forest') || club.includes('burnley') || club.includes('luton') ||
        club.includes('sheffield united') || club.includes('bournemouth')) {
      return 'Premier League';
    } else if (club.includes('barcelona') || club.includes('madrid') || club.includes('atletico')) {
      return 'La Liga';
    } else if (club.includes('bayern') || club.includes('dortmund') || club.includes('leipzig')) {
      return 'Bundesliga';
    } else if (club.includes('inter') || club.includes('milan') || club.includes('juventus')) {
      return 'Serie A';
    } else if (club.includes('psg') || club.includes('monaco') || club.includes('nice')) {
      return 'Ligue 1';
    }
  ...
  generateTags(club, league, category, season) {
    ...
    if (club.includes('Arsenal') || club.includes('Manchester United') || club.includes('Real Madrid')) {
      tags.push('Adidas');
    } else if (club.includes('Barcelona') || club.includes('PSG') || club.includes('Chelsea')) {
      tags.push('Nike');
  ...
  generatePrice(club, category) {
    ...
    const premiumClubs = ['Real Madrid', 'Barcelona', 'Manchester United', 'Liverpool', 'PSG'];
  ...
  enrichAlbum(album) {
    ...
    return {
      ...
      inStock: true,
      brand: tags.find(tag => ['Adidas', 'Nike', 'Puma', 'Under Armour'].includes(tag)) || 'Adidas',
